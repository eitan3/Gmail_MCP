...
def register(mcp) -> None:
    ...
    @mcp.tool()
    def untrash_message(message_id: str, account: str | None = None) -> dict:
        """Restore a message from Trash."""
        c = client_for(account)
        return c.execute(c.users.messages().untrash(userId="me", id=message_id))

    @mcp.tool()
    def trash_thread(thread_id: str, account: str | None = None) -> dict:
        """Move an entire thread to Trash."""
        c = client_for(account)
        return c.execute(c.users.threads().trash(userId="me", id=thread_id))

    @mcp.tool()
    def untrash_thread(thread_id: str, account: str | None = None) -> dict:
